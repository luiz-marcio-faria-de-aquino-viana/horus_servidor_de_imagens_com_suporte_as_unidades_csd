...
config_tag_t* CCfgConfigTag::addNewConfigTag(int n, int tagId, char* tagName)
{
    ...
    debugEntry(DEBUG_LEVEL_03, __HORUSSRV_CFG_CONFIG_H, "addNewConfigTag()", p);
    return p;
}
...
config_tag_t* CCfgConfigTag::setConfigTagValue(char* tagName, char* tagValue)
{
    for(long i = 0; i < m_currNumConfigTags; i++) {
        ...
        if( strNCmpCaseUtil(p->tag_name, tagName, BIGSTRSZ, DEF_CASESENSITIVE_TOUPPER) == 0 ) {
            ...
            debugEntry(DEBUG_LEVEL_03, __HORUSSRV_CFG_CONFIG_H, "setConfigTagValue()", p);
            return p;
        }
    }
    return NULL;
...
void CCfgConfigTag::loadConfigTagsValue(char* cfgfile)
{
    ...
    int rscode = openFileUtil(&f, cfgfile, (char*)FILMODE_READ_ASCII, DBTRUE);
    if(rscode == RSOK)
    {
        long rowNum = 0;
        while(fgets(sbuf, BIGSTRSZ, f) != NULL) {
            ...
            if(tk != NULL) {
                ...
                warnMsg(DEBUG_LEVEL_03, __HORUSSRV_CFG_CONFIG_H, "loadConfigTagsValue()", errmsg);
