...
void CCmdListener::start()
{
    ...
    if(rscode == -1)
        errMsg(__HORUSWRK_CMD_LISTENER_H, "start()", ERR_CANTCREATELISTNERTHREAD);
    sleep(3);
}
...
void CCmdListener::stop()
{
    if(m_threadHnd != 0)
    {
        sendStringMessageToCurrRemoteUnit((char*)DEF_DEFAULT_SERVERTERMMSG);
...
int CCmdListener::sendStringMessageToCurrRemoteUnit(char* data)
{
    int n = (sizeof(char) * strlen(data)) + 1;
    int rscode = sendDataMessageToCurrRemoteUnit((byte*)data, n);
    return rscode;
}

int CCmdListener::sendDataMessageToCurrRemoteUnit(byte* data, int n)
{
    CCfgConfig* pConfig = gAppMain.getConfigPtr();

    CCfgRemoteUnit* pCurrRemoteUnit = pConfig->getCurrRemoteUnitPtr();

    int rscode = sendDataMessageToAny(pCurrRemoteUnit->getIpAddr(), pCurrRemoteUnit->getInPort(), (byte*)data, n);
    return rscode;
}
...
int CCmdListener::sendDataMessageToAny(char* dst_addr, int dst_port, byte* data, int n)
{
    ...
    bigstr_t errmsg;
    sprintf(errmsg, "\nCCmdListener::SendDataMessage: DstAddr=%s, DstPort=%d", dst_addr, dst_port);
    warnMsg(DEBUG_LEVEL_01, __HORUSWRK_CMD_LISTENER_H, "sendDataMessageToAny()", errmsg);
...
int CCmdListener::sendDataMessageToCon(connection_t* pCon, byte* data, int n)
{
    ...
    saddr.sin_family = AF_INET;
    saddr.sin_port = htons(pCurrRemoteUnit->getOutPort());
...
int CCmdListener::sendDataMessageToINET(struct sockaddr_in* p_saddr, long saddrsz, byte* data, int n)
{
	...
	if(clientHnd == -1) {
        errMsg(__HORUSWRK_CMD_LISTENER_H, "sendDataMessageToINET()", ERR_CANTCREATESOCKET);
        return RSERR;
    }

    if (connect(clientHnd, (struct sockaddr*) p_saddr, saddrsz) == -1) {
        errMsg(__HORUSWRK_CMD_LISTENER_H, "sendDataMessageToINET()", ERR_CANTCONNECTSOCKET);
        return RSERR;
    }

    if(send(clientHnd, data, n, MSG_DONTWAIT) == -1) {
        errMsg(__HORUSWRK_CMD_LISTENER_H, "sendDataMessageToINET()", ERR_CANTSENDMESSAGE);
        return RSERR;
    }
...
extern void* cmd_listener_exec(void* _pArgs)
{
    ...
    byte dataType = pCon->in_buf[0];

    if( IS_DSK_DATATYPE(dataType) ) {
        printf("\n => Processing Disk Data Request...\n");
        pReq = new CCmdRequest(pCon->reqnum, pCon);
    }
    else {
    ...
    if(pReq->getAction() != DEF_REQ_ACTION_UNKNOW_I)
    {
        ...
        if(execWorker.doExec() == RSOK) {
            ...
            int rscode = RSERR;
            if(pReq->getRemoteUnitOid() == -1)
                rscode = gCmdListener.sendStringMessageToHost(outputXml);
            else
                rscode = gCmdListener.sendStringMessageToRemoteUnit(pReq->getRemoteUnitOid(), outputXml);
            if(rscode == RSERR)
                errMsg(__HORUSWRK_CMD_LISTENER_H, "cmd_listener_exec()", ERR_CANTSENDMESSAGE);
        }
    }
...
extern void* cmd_listener_run(void* _pArgs)
{
    ...
    gCmdListener.setIpAddr(pCurrRemoteUnit->getIpAddr());
    gCmdListener.setInPort(pCurrRemoteUnit->getInPort());
    gCmdListener.setOutPort(pCurrRemoteUnit->getOutPort());
    ...
    warnMsg(DEBUG_LEVEL_01, __HORUSWRK_CMD_LISTENER_H, "cmd_listener_run()", errmsg);

	int serverHnd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
	if(serverHnd == -1) 
        errMsg(__HORUSWRK_CMD_LISTENER_H, "cmd_listener_run()", ERR_CANTCREATESOCKET);

    int sockreuse = 1;
    if(setsockopt(serverHnd, SOL_SOCKET, SO_REUSEADDR, &sockreuse, sizeof(int)) == -1)
        errMsg(__HORUSWRK_CMD_LISTENER_H, "cmd_listener_run()", ERR_CANTSETSOCKETOPT);

    if(bind(serverHnd, (struct sockaddr*)&saddr, sizeof(saddr)) == -1)
        errMsg(__HORUSWRK_CMD_LISTENER_H, "cmd_listener_run()", ERR_CANTBINDSOCKET);
    ...
    while(gCmdListener.isRunning() == TRUE)
    {
        ...
        if(in_len != -1) 
        {
            ...
            if(pCon != NULL)
            {
                ...
                if(rscode == -1)
                    errMsg(__HORUSWRK_CMD_LISTENER_H, "cmd_listener_run()", ERR_CANTCREATELISTNERTHREAD);
            }
        }
    }
